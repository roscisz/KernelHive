...
DataDownloaderTCP::DataDownloaderTCP(NetworkAddress* address, const char* dataId, SynchronizedBuffer* buffer)
	: IDataDownloader(address, dataId, buffer)
{
...
void DataDownloaderTCP::onMessage(TCPMessage* message) {
	...
	case STATE_INITIAL:
		if (acquireDataSize(message)) {
			...
			tcpClient->sendMessage(dataQuery);
		} else {
			tcpClient->sendMessage(sizeQuery);
		}
		break;
...
void DataDownloaderTCP::onConnected() {
	Logger::log(INFO, "Connection established\n");
	tcpClient->sendMessage(sizeQuery);
}

DataDownloaderTCP::~DataDownloaderTCP() {
	if (sizeQuery != NULL) {
		delete sizeQuery;
	}
	if (dataQuery != NULL) {
...
bool DataDownloaderTCP::acquireDataSize(TCPMessage *sizeMessage) {
	bool outcome = false;
...
void DataDownloaderTCP::prepareQueries() {
...
void DataDownloaderTCP::run() {
	tcpClient->run();
}

void DataDownloaderTCP::pleaseStop() {
