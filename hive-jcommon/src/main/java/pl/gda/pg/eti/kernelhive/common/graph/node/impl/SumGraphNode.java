...
public class SumGraphNode extends GenericGraphNode {

	public SumGraphNode(String id) {
		super(id);
		type = GraphNodeType.SUM;
	}
	...
	public boolean canAddPreviousNode(IGraphNode node) {
		return previousNodes.size() < 2 && node.getType().isImageType();
	}
	...
	public List<ValidationResult> validate() {
		...
		// previous nodes? (must be less than 3)
		if (getPreviousNodes() != null && getPreviousNodes().size() > 2) {
			results.add(new ValidationResult("Node (id: " + nodeId + ", name: "
					+ name + ") of type '" + type
