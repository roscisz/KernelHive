...
public class RepositoryLoaderService {
	...
	private static final String KERNEL_REPOSITORY_CLASS_NAME = 
			"pl.gda.pg.eti.kernelhive.repository.kernel.repository.KernelRepository";
	private static final String GRAPH_NODE_BUILDER_CLASS_NAME = 
			"pl.gda.pg.eti.kernelhive.repository.graph.node.GraphNodeBuilder";
	private static final String GRAPH_NODE_TYPE_FACTORY_CLASS_NAME = 
			"pl.gda.pg.eti.kernelhive.repository.graph.node.type.GraphNodeTypeFactory";
	...
	@SuppressWarnings("rawtypes")
	public IGraphNodeTypeFactory createGraphNodeTypeFactory() {
		IGraphNodeTypeFactory factory = null;
		for(Class clazz : loadedClasses){
			if(clazz.getName().equals(GRAPH_NODE_TYPE_FACTORY_CLASS_NAME)){
				try {
					factory = (IGraphNodeTypeFactory) clazz.newInstance();
				} catch (InstantiationException e) {
					e.printStackTrace();
				} catch (IllegalAccessException e) {
					e.printStackTrace();
